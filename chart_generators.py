...
def get_responsive_config():
    """Returns responsive configuration for all charts"""
    return {
        'responsive': True,
...
def create_measles_timeline(timeline_data):
    """Mobile-responsive timeline chart"""
    import numpy as np
    import pandas as pd
    import plotly.graph_objects as go
    ...
    def wrap_text(text, width=20):  # Shorter for mobile
    ...
    if has_highlights.any():
        ...
        if not national_events.empty:
            fig.add_trace(go.Scatter(
                ...
                mode="markers",
                marker=dict(size=12, color=colors['national_marker'], symbol="circle",
                           line=dict(color=colors['primary_line'], width=2)),
                hovertemplate="<b>%{x}</b><br>Cases: %{customdata:,}<br>%{text}<extra></extra>",
        ...
        if not arizona_events.empty:
            fig.add_trace(go.Scatter(
                ...
                mode="markers",
                marker=dict(size=12, color=colors['arizona_marker'], symbol="diamond",
                           line=dict(color=colors['primary_line'], width=2)),
                hovertemplate="<b>%{x}</b><br>Cases: %{customdata:,}<br>%{text}<extra></extra>",
    ...
    # Footer
    fig.add_annotation(
        text=f"Last refreshed: {datetime.now().strftime('%B %d, %Y')}<br>Note: Square-root scale used",
...
def create_recent_trends(usmeasles_data, mmr_data):
    """Mobile-responsive recent trends chart"""
    import pandas as pd
    import plotly.graph_objects as go
    ...
    if 'MMR' in us_data.columns and not us_data['MMR'].isna().all():
        ...
        if not valid_vaccination.empty:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=10, color=colors['orange'], line=dict(color='white', width=2)),
    ...
    fig.update_layout(
        ...
        paper_bgcolor='white',
        font=dict(family=FONT_FAMILY, size=12, color='black'),
        margin=dict(l=50, r=60 if has_vaccination_data else 20, t=80, b=100),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
    ...
    # Footer
    fig.add_annotation(
        text=f"Last refreshed: {datetime.now().strftime('%B %d, %Y')}",
        xref="paper", yref="paper",
        x=0, y=-0.18,
        xanchor="left", yanchor="top",
        showarrow=False,
...
def create_rnaught_comparison():
    """Mobile-responsive R₀ comparison"""
    import numpy as np
    import pandas as pd
    import plotly.graph_objects as go
    import math
    ...
    diseases_data = {
        'Disease': ['Ebola', 'HIV', 'COVID-19\n(Omicron)', 'Chickenpox', 'Mumps', 'Measles'],
        'R0': [2, 4, 9.5, 12, 14, 18]
    }
    ...
    for i, (disease, r0) in enumerate(zip(df['Disease'], df['R0'])):
        ...
        for j in range(TOTAL_DOTS):
            dot_color = INFECTED_COLOR if j < r0 else NOT_INFECTED_COLOR
            hover_text = f"{disease}: {'Infected' if j < r0 else 'Not infected'}"

            fig.add_trace(go.Scatter(
                x=[x_coords[j]],
                y=[y_coords[j]],
                mode='markers',
                marker=dict(size=DOT_SIZE, color=dot_color, line=dict(width=1, color='white')),
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[cy],
            mode='markers',
            marker=dict(size=CENTER_DOT_SIZE, color=INDEX_CASE_COLOR, line=dict(color='white', width=2)),
            hovertemplate=f"<b>Index case</b><br>{disease} (R₀={r0})<extra></extra>",
            showlegend=False
        ))
    ...
    x_min = -CIRCLE_RADIUS - 0.5
    x_max = (len(df) - 1) * X_SPACING + CIRCLE_RADIUS + 0.5
    y_min = Y_POSITION - CIRCLE_RADIUS - 2
    y_max = Y_POSITION + CIRCLE_RADIUS + 0.5
    ...
    # Legend
    fig.add_annotation(
        text='Each circle = 20 people. <span style="color:#fdae61">●</span> Index case, <span style="color:#d73027">●</span> Infected, <span style="color:#d3d3d3">●</span> Not infected',
        xref="paper", yref="paper",
        x=0, y=1.1,
        xanchor="left", yanchor="top",
        showarrow=False,
...
def create_bivariate_choropleth(usmap_data):
    """Mobile-responsive bivariate choropleth with adaptive labels"""
    import plotly.graph_objects as go
    import pandas as pd
    import numpy as np
    from datetime import datetime
    ...
    category_labels = [
        ["High Cases, Low Vacc", "High Cases, Med Vacc", "High Cases, High Vacc"],
        ["Med Cases, Low Vacc", "Med Cases, Med Vacc", "Med Cases, High Vacc"],
        ["Low Cases, Low Vacc", "Low Cases, Med Vacc", "Low Cases, High Vacc"]
    ]
    ...
    if not missing_mmr_states_df.empty:
        fig.add_trace(go.Choropleth(
            ...
            hovertemplate=(
                '<b>%{customdata[0]}</b><br>'
                'Missing Data<br>'
                'Cases: %{customdata[1]:.1f}/100K<extra></extra>'
            ),
            customdata=list(zip(
            ...
            if len(subset) > 0:
                fig.add_trace(go.Choropleth(
                    ...
                    hovertemplate=(
                        '<b>%{customdata[0]}</b><br>'
                        '%{customdata[4]}<br>'
                        'MMR: %{customdata[1]:.1f}%<br>'
                        'Cases: %{customdata[2]:.1f}/100K<extra></extra>'
                    ),
                    customdata=list(zip(
                        subset['geography'],
    ...
    fig.add_shape(
        ...
        xref="paper", yref="paper",
        x0=legend_x,
        y0=legend_y - 4 * (cell_size + spacing),
        x1=legend_x + cell_size,
        y1=legend_y - 3 * (cell_size + spacing),
    ...
    # Add state labels - smaller for mobile
    ...
    if white_text_states:
        fig.add_trace(go.Scattergeo(
            ...
            mode='text',
            textfont=dict(size=label_font_size, color='white', family='Arial'),
            showlegend=False,
            hoverinfo='skip'
        ))
    ...
    # Footer
    fig.add_annotation(
        text=f"Last refreshed: {datetime.now().strftime('%B %d, %Y')}<br>Grey states missing vaccination data",
        xref="paper", yref="paper",
        x=0.02, y=0.02,
        showarrow=False,
        font=dict(size=9, color='gray'),
...
def create_lives_saved_chart(vaccine_impact_data):
    """Mobile-responsive lives saved chart"""
    import plotly.graph_objects as go
    import pandas as pd
    import numpy as np
    import math
    ...
    fig.add_annotation(
        text="Range: ",
        xref="paper", yref="paper",
        x=0.01, y=legend_y + cell_height/2,
        showarrow=False,
    ...
    display_bin_indices = np.linspace(0, len(bin_labels) - 1, min(len(bin_labels), 6), dtype=int)
    ...
    for i in display_bin_indices:
        ...
        estimated_label_width = len(label_text) * 0.007
        current_x = label_x_position + estimated_label_width + spacing

    # Footer
    fig.add_annotation(
        text=f"Last refreshed: {datetime.now().strftime('%B %d, %Y')}<br>Note: Model estimates",
        xref="paper", yref="paper",
        x=0, y=-0.18,
        xanchor="left", yanchor="top",
        showarrow=False,
...
def create_southwest_weekly_comparison(weekly_data):
    """Mobile-responsive Southwest weekly comparison"""
    import plotly.graph_objects as go
    import pandas as pd
    ...
    current_week_str = current_week_start.strftime('%m/%d')

    if not previous_df.empty:
        last_week_start = current_week_start - timedelta(days=7)
        last_week_str = last_week_start.strftime('%m/%d')
    else:
        last_week_str = 'N/A'
    ...
    if not previous_df.empty:
        ...
        def format_change(row):
            if row['Cases_last'] == 0 and row['Cases_current'] == 0:
                ...
                return f"↑ +{row['Change_Percent']:.0f}%"
            elif row['Change_Value'] < 0:
                return f"↓ {row['Change_Percent']:.0f}%"
            else:
                return "→"
    ...
    header_values = [
        '<b>State</b>',
        f'<b>Week of {current_week_str}</b>',
        f'<b>Week of {last_week_str}</b>',
        '<b>Change</b>',
        '<b>Info</b>'
    ]
    ...
    for state in comparison['State']:
        if state in state_urls:
            website_links.append(f'<a href="{state_urls[state]}">Link ↗</a>')
        else:
    ...
    fig.add_annotation(
        text=f"Last refreshed: {datetime.now().strftime('%B %d, %Y')}<br>Data from CDC website",
        xref="paper", yref="paper",
        x=0, y=-0.12,
        xanchor="left", yanchor="top",
        showarrow=False,
